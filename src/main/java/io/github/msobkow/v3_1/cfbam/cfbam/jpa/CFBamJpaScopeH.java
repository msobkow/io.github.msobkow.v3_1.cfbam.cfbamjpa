// Description: Java 25 JPA implementation of Scope history objects
...
/**
 *  CFBamJpaScopeH provides history objects matching the CFBamScope change history.
 */
@Entity
@Table(
    name = "scopedef_h", schema = "CFBam31",
    indexes = {
        @Index(name = "ScopeIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "ScopeTenantIdx_h", columnList = "TenantId", unique = false)
    }
)
    ...
    implements ICFBamScopeH, Comparable<Object>, Serializable
{
    ...
    @EmbeddedId
    protected CFBamJpaScopeHPKey pkey;
	@AttributeOverrides({
    ...
    public CFBamJpaScopeH() {
            // The primary key member attributes are initialized on construction
            pkey = new CFBamJpaScopeHPKey();
    ...
    public void setPKey( ICFBamScopeHPKey pkey ) {
        if (pkey != null) {
            if (pkey instanceof CFBamJpaScopeHPKey) {
                this.pkey = (CFBamJpaScopeHPKey)pkey;
            }
            else {
                throw new CFLibUnsupportedClassException(getClass(), "setPKey", "pkey", pkey, "CFBamJpaScopeHPKey");
            }
        }
    }
    ...
    public String toString() {
        String ret = "<CFBamJpaScopeH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}

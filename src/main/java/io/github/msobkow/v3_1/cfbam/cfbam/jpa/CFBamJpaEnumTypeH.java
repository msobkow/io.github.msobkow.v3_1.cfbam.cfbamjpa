// Description: Java 25 JPA implementation of EnumType history objects
...
/**
 *  CFBamJpaEnumTypeH provides history objects matching the CFBamEnumType change history.
 */
@Entity
@Table(
    name = "enumtyp_h", schema = "CFBam31",
    indexes = {
        @Index(name = "EnumTypeIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "EnumTypeSchemaDefIdx_h", columnList = "SchemaDefId", unique = false)
    }
)
@Inheritance(strategy = InheritanceType.JOINED)
@DiscriminatorValue("43120")
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFBamPU")
public class CFBamJpaEnumTypeH extends CFBamJpaEnumDefH
    implements ICFBamEnumTypeH
{
    ...
    public void set( ICFBamValue src ) {
		if( src instanceof ICFBamEnumType ) {
			setEnumType( (ICFBamEnumType)src );
		}
		else {
    ...
    public String toString() {
        String ret = "<CFBamJpaEnumTypeH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}

// Description: Java 25 JPA implementation of Atom history objects
...
/**
 *  CFBamJpaAtomH provides history objects matching the CFBamAtom change history.
 */
@Entity
@Table(
    name = "atomdef_h", schema = "CFBam31",
    indexes = {
        @Index(name = "AtomIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true)
    }
)
@Inheritance(strategy = InheritanceType.JOINED)
@DiscriminatorValue("43018")
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFBamPU")
public class CFBamJpaAtomH extends CFBamJpaValueH
    implements ICFBamAtomH
{
    ...
    public void set( ICFBamValue src ) {
		if( src instanceof ICFBamAtom ) {
			setAtom( (ICFBamAtom)src );
		}
		else {
    ...
    public String toString() {
        String ret = "<CFBamJpaAtomH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}

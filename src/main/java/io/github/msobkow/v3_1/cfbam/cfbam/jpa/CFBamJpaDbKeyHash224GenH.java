// Description: Java 25 JPA implementation of DbKeyHash224Gen history objects
...
/**
 *  CFBamJpaDbKeyHash224GenH provides history objects matching the CFBamDbKeyHash224Gen change history.
 */
@Entity
@Table(
    name = "idgdbk224_h", schema = "CFBam31",
    indexes = {
        @Index(name = "DbKeyHash224GenIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true)
    }
)
@Inheritance(strategy = InheritanceType.JOINED)
@DiscriminatorValue("43075")
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFBamPU")
public class CFBamJpaDbKeyHash224GenH extends CFBamJpaDbKeyHash224TypeH
    implements ICFBamDbKeyHash224GenH
{
    ...
    public void set( ICFBamValue src ) {
		if( src instanceof ICFBamDbKeyHash224Gen ) {
			setDbKeyHash224Gen( (ICFBamDbKeyHash224Gen)src );
		}
		else {
    ...
    public String toString() {
        String ret = "<CFBamJpaDbKeyHash224GenH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}

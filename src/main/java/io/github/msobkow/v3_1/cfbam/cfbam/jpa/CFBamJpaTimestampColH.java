// Description: Java 25 JPA implementation of TimestampCol history objects
...
/**
 *  CFBamJpaTimestampColH provides history objects matching the CFBamTimestampCol change history.
 */
@Entity
@Table(
    name = "tscol_h", schema = "CFBam31",
    indexes = {
        @Index(name = "TimestampColIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "TimestampColTableIdx_h", columnList = "TableId", unique = false)
    }
)
@Inheritance(strategy = InheritanceType.JOINED)
@DiscriminatorValue("43137")
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFBamPU")
public class CFBamJpaTimestampColH extends CFBamJpaTimestampDefH
    implements ICFBamTimestampColH
{
    ...
    public void set( ICFBamValue src ) {
		if( src instanceof ICFBamTimestampCol ) {
			setTimestampCol( (ICFBamTimestampCol)src );
		}
		else {
    ...
    public String toString() {
        String ret = "<CFBamJpaTimestampColH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}

// Description: Java 25 JPA implementation of DbKeyHash224Def history objects
...
/**
 *  CFBamJpaDbKeyHash224DefH provides history objects matching the CFBamDbKeyHash224Def change history.
 */
@Entity
@Table(
    name = "dbk224def_h", schema = "CFBam31",
    indexes = {
        @Index(name = "DbKeyHash224DefIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true)
    }
)
@Inheritance(strategy = InheritanceType.JOINED)
@DiscriminatorValue("43073")
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFBamPU")
public class CFBamJpaDbKeyHash224DefH extends CFBamJpaAtomH
    implements ICFBamDbKeyHash224DefH
{
	@Column( name="InitVal", nullable=true, length=56 )
	protected String optionalInitValue;

    public CFBamJpaDbKeyHash224DefH() {
            super();
    ...
    public void set( ICFBamValue src ) {
		if( src instanceof ICFBamDbKeyHash224Def ) {
			setDbKeyHash224Def( (ICFBamDbKeyHash224Def)src );
		}
		else {
    ...
    public String toString() {
        String ret = "<CFBamJpaDbKeyHash224DefH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}

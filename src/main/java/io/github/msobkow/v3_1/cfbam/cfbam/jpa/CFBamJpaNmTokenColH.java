// Description: Java 25 JPA implementation of NmTokenCol history objects
...
/**
 *  CFBamJpaNmTokenColH provides history objects matching the CFBamNmTokenCol change history.
 */
@Entity
@Table(
    name = "nmtkncol_h", schema = "CFBam31",
    indexes = {
        @Index(name = "NmTokenColIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "NmTokenColTableIdx_h", columnList = "TableId", unique = false)
    }
)
@Inheritance(strategy = InheritanceType.JOINED)
@DiscriminatorValue("43128")
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFBamPU")
public class CFBamJpaNmTokenColH extends CFBamJpaNmTokenDefH
    implements ICFBamNmTokenColH
{
    ...
    public void set( ICFBamValue src ) {
		if( src instanceof ICFBamNmTokenCol ) {
			setNmTokenCol( (ICFBamNmTokenCol)src );
		}
		else {
    ...
    public String toString() {
        String ret = "<CFBamJpaNmTokenColH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}

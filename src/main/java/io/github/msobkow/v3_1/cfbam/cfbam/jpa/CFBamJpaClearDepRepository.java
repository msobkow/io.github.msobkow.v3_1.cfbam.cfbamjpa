// Description: Java 25 Spring JPA Repository for ClearDep
...
/**
 *	JpaRepository for the CFBamJpaClearDep entities defined in io.github.msobkow.v3_1.cfbam.cfbam.jpa.
 *	The manufactured repositories try to provide a rich, do-it-all interface to the JPA data store, with both object and argument-based implementations of the interface defined.
 */
@Transactional(readOnly = true)
public interface CFBamJpaClearDepRepository extends JpaRepository<CFBamJpaClearDep, CFLibDbKeyHash256> {
	...
	CFBamJpaClearDep get(@Param("id") CFLibDbKeyHash256 requiredId);
	 ...
	 */
	default List<CFBamJpaClearDep> findByTenantIdx(ICFBamScopeByTenantIdxKey key) {
	...
	List<CFBamJpaClearDep> findByClearDepIdx(@Param("relationId") CFLibDbKeyHash256 requiredRelationId);
	 ...
	 *		@return The found entity list, which may be empty, typically populated from the JPA cache.
	 */
	default List<CFBamJpaClearDep> findByClearDepIdx(ICFBamClearDepByClearDepIdxKey key) {
	...
	List<CFBamJpaClearDep> findByDefSchemaIdx(@Param("defSchemaId") CFLibDbKeyHash256 optionalDefSchemaId);
	 ...
	 *		@return The found entity list, which may be empty, typically populated from the JPA cache.
	 */
	default List<CFBamJpaClearDep> findByDefSchemaIdx(ICFBamClearDepByDefSchemaIdxKey key) {
	...
	@Query("select r from CFBamJpaScope r where r.requiredId = :id")
	CFBamJpaClearDep lockByIdIdx(@Param("id") CFLibDbKeyHash256 requiredId);
	 ...
	 *		@return A list of locked entities, refreshed from the data store, or an empty list if no such entities exist.
	 */
	default List<CFBamJpaClearDep> lockByTenantIdx(ICFBamScopeByTenantIdxKey key) {
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFBamJpaClearDep r where r.requiredLookupRelation.requiredId = :relationId")
	List<CFBamJpaClearDep> lockByClearDepIdx(@Param("relationId") CFLibDbKeyHash256 requiredRelationId);
	 ...
	 *		@return A list of locked entities, refreshed from the data store, or an empty list if no such entities exist.
	 */
	default List<CFBamJpaClearDep> lockByClearDepIdx(ICFBamClearDepByClearDepIdxKey key) {
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFBamJpaClearDep r where r.optionalLookupDefSchema.requiredId = :defSchemaId")
	List<CFBamJpaClearDep> lockByDefSchemaIdx(@Param("defSchemaId") CFLibDbKeyHash256 optionalDefSchemaId);
	 ...
	 *		@return A list of locked entities, refreshed from the data store, or an empty list if no such entities exist.
	 */
	default List<CFBamJpaClearDep> lockByDefSchemaIdx(ICFBamClearDepByDefSchemaIdxKey key) {

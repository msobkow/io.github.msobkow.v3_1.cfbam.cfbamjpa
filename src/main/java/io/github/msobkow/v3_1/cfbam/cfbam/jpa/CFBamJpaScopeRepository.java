// Description: Java 25 Spring JPA Repository for Scope
...
/**
 *	JpaRepository for the CFBamJpaScope entities defined in io.github.msobkow.v3_1.cfbam.cfbam.jpa.
 *	The manufactured repositories try to provide a rich, do-it-all interface to the JPA data store, with both object and argument-based implementations of the interface defined.
 */
@Transactional(readOnly = true)
public interface CFBamJpaScopeRepository extends JpaRepository<CFBamJpaScope, CFLibDbKeyHash256> {
	...
	CFBamJpaScope get(@Param("id") CFLibDbKeyHash256 requiredId);
	 ...
	 *
	 *		@return List&lt;CFBamJpaScope&gt; of the found entities, typically from the JPA cache, or an empty list if no such entities exist.
	 */
	@Query("select r from CFBamJpaScope r where r.requiredTenantId = :tenantId")
	List<CFBamJpaScope> findByTenantIdx(@Param("tenantId") CFLibDbKeyHash256 requiredTenantId);
	 ...
	 */
	default List<CFBamJpaScope> findByTenantIdx(ICFBamScopeByTenantIdxKey key) {
	...
	@Query("select r from CFBamJpaScope r where r.requiredId = :id")
	CFBamJpaScope lockByIdIdx(@Param("id") CFLibDbKeyHash256 requiredId);
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFBamJpaScope r where r.requiredTenantId = :tenantId")
	List<CFBamJpaScope> lockByTenantIdx(@Param("tenantId") CFLibDbKeyHash256 requiredTenantId);
	 ...
	 *		@return A list of locked entities, refreshed from the data store, or an empty list if no such entities exist.
	 */
	default List<CFBamJpaScope> lockByTenantIdx(ICFBamScopeByTenantIdxKey key) {

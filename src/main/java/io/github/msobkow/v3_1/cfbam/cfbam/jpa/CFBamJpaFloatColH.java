// Description: Java 25 JPA implementation of FloatCol history objects
...
/**
 *  CFBamJpaFloatColH provides history objects matching the CFBamFloatCol change history.
 */
@Entity
@Table(
    name = "fltcol_h", schema = "CFBam31",
    indexes = {
        @Index(name = "FloatColIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "FloatColTableIdx_h", columnList = "TableId", unique = false)
    }
)
...
public class CFBamJpaFloatColH extends CFBamJpaFloatDefH
    implements ICFBamFloatColH
{
    ...
    public void set( ICFBamValue src ) {
		if( src instanceof ICFBamFloatCol ) {
			setFloatCol( (ICFBamFloatCol)src );
		}
		else {
    ...
    public String toString() {
        String ret = "<CFBamJpaFloatColH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}

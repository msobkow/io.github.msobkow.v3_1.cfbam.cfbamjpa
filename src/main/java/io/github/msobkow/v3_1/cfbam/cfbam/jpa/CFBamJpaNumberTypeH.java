// Description: Java 25 JPA implementation of NumberType history objects
...
/**
 *  CFBamJpaNumberTypeH provides history objects matching the CFBamNumberType change history.
 */
@Entity
@Table(
    name = "numtyp_h", schema = "CFBam31",
    indexes = {
        @Index(name = "NumberTypeIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "NumberTypeSchemaDefIdx_h", columnList = "SchemaDefId", unique = false)
    }
)
@Inheritance(strategy = InheritanceType.JOINED)
@DiscriminatorValue("43054")
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFBamPU")
public class CFBamJpaNumberTypeH extends CFBamJpaNumberDefH
    implements ICFBamNumberTypeH
{
    ...
    public void set( ICFBamValue src ) {
		if( src instanceof ICFBamNumberType ) {
			setNumberType( (ICFBamNumberType)src );
		}
		else {
    ...
    public String toString() {
        String ret = "<CFBamJpaNumberTypeH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}

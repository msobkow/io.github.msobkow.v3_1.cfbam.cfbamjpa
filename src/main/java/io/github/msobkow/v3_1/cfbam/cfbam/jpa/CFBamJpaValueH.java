// Description: Java 25 JPA implementation of Value history objects
...
/**
 *  CFBamJpaValueH provides history objects matching the CFBamValue change history.
 */
@Entity
@Table(
    name = "valdef_h", schema = "CFBam31",
    indexes = {
        @Index(name = "ValueIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "ValueUNameIdx_h", columnList = "ScopeId, safe_name", unique = true),
        @Index(name = "ValueScopeIdx_h", columnList = "ScopeId", unique = false),
        @Index(name = "ValueDefSchemaDefIdx_h", columnList = "defschid", unique = false),
        @Index(name = "ValuePrevIdx_h", columnList = "PrevId", unique = false),
        @Index(name = "ValueNextIdx_h", columnList = "NextId", unique = false),
        @Index(name = "ValueContPrevIdx_h", columnList = "ScopeId, PrevId", unique = false),
        @Index(name = "ValueContNextIdx_h", columnList = "ScopeId, NextId", unique = false)
    }
)
    ...
    implements ICFBamValueH, Comparable<Object>, Serializable
{
    ...
    @EmbeddedId
    protected CFBamJpaValueHPKey pkey;
	@AttributeOverrides({
    ...
    public void setPKey( ICFBamValueHPKey pkey ) {
        if (pkey != null) {
            if (pkey instanceof CFBamJpaValueHPKey) {
                this.pkey = (CFBamJpaValueHPKey)pkey;
            }
            else {
                throw new CFLibUnsupportedClassException(getClass(), "setPKey", "pkey", pkey, "CFBamJpaValueHPKey");
            }
        }
    }
    ...
    public String toString() {
        String ret = "<CFBamJpaValueH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}

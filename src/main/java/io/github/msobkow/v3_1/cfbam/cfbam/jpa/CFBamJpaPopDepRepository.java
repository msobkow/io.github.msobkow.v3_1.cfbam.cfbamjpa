// Description: Java 25 Spring JPA Repository for PopDep
...
/**
 *	JpaRepository for the CFBamJpaPopDep entities defined in io.github.msobkow.v3_1.cfbam.cfbam.jpa.
 *	The manufactured repositories try to provide a rich, do-it-all interface to the JPA data store, with both object and argument-based implementations of the interface defined.
 */
@Transactional(readOnly = true)
public interface CFBamJpaPopDepRepository extends JpaRepository<CFBamJpaPopDep, CFLibDbKeyHash256> {
	...
	CFBamJpaPopDep get(@Param("id") CFLibDbKeyHash256 requiredId);
	 ...
	 *
	 *		@return List&lt;CFBamJpaPopDep&gt; of the found entities, typically from the JPA cache, or an empty list if no such entities exist.
	 */
	@Query("select r from CFBamJpaScope r where r.requiredTenantId = :tenantId")
	List<CFBamJpaPopDep> findByTenantIdx(@Param("tenantId") CFLibDbKeyHash256 requiredTenantId);
	 ...
	 */
	default List<CFBamJpaPopDep> findByTenantIdx(ICFBamScopeByTenantIdxKey key) {
	...
	List<CFBamJpaPopDep> findByRelationIdx(@Param("relationId") CFLibDbKeyHash256 requiredRelationId);
	 ...
	 *		@return The found entity list, which may be empty, typically populated from the JPA cache.
	 */
	default List<CFBamJpaPopDep> findByRelationIdx(ICFBamPopDepByRelationIdxKey key) {
	...
	List<CFBamJpaPopDep> findByDefSchemaIdx(@Param("defSchemaId") CFLibDbKeyHash256 optionalDefSchemaId);
	 ...
	 *		@return The found entity list, which may be empty, typically populated from the JPA cache.
	 */
	default List<CFBamJpaPopDep> findByDefSchemaIdx(ICFBamPopDepByDefSchemaIdxKey key) {
	...
	@Query("select r from CFBamJpaScope r where r.requiredId = :id")
	CFBamJpaPopDep lockByIdIdx(@Param("id") CFLibDbKeyHash256 requiredId);
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFBamJpaScope r where r.requiredTenantId = :tenantId")
	List<CFBamJpaPopDep> lockByTenantIdx(@Param("tenantId") CFLibDbKeyHash256 requiredTenantId);
	 ...
	 *		@return A list of locked entities, refreshed from the data store, or an empty list if no such entities exist.
	 */
	default List<CFBamJpaPopDep> lockByTenantIdx(ICFBamScopeByTenantIdxKey key) {
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFBamJpaPopDep r where r.requiredLookupRelation.requiredId = :relationId")
	List<CFBamJpaPopDep> lockByRelationIdx(@Param("relationId") CFLibDbKeyHash256 requiredRelationId);
	 ...
	 *		@return A list of locked entities, refreshed from the data store, or an empty list if no such entities exist.
	 */
	default List<CFBamJpaPopDep> lockByRelationIdx(ICFBamPopDepByRelationIdxKey key) {
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFBamJpaPopDep r where r.optionalLookupDefSchema.requiredId = :defSchemaId")
	List<CFBamJpaPopDep> lockByDefSchemaIdx(@Param("defSchemaId") CFLibDbKeyHash256 optionalDefSchemaId);
	 ...
	 *		@return A list of locked entities, refreshed from the data store, or an empty list if no such entities exist.
	 */
	default List<CFBamJpaPopDep> lockByDefSchemaIdx(ICFBamPopDepByDefSchemaIdxKey key) {

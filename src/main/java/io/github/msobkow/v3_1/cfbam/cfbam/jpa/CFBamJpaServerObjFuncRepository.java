// Description: Java 25 Spring JPA Repository for ServerObjFunc
...
/**
 *	JpaRepository for the CFBamJpaServerObjFunc entities defined in io.github.msobkow.v3_1.cfbam.cfbam.jpa.
 *	The manufactured repositories try to provide a rich, do-it-all interface to the JPA data store, with both object and argument-based implementations of the interface defined.
 */
@Transactional(readOnly = true)
public interface CFBamJpaServerObjFuncRepository extends JpaRepository<CFBamJpaServerObjFunc, CFLibDbKeyHash256> {
	...
	CFBamJpaServerObjFunc get(@Param("id") CFLibDbKeyHash256 requiredId);
	 ...
	 *
	 *		@return List&lt;CFBamJpaServerObjFunc&gt; of the found entities, typically from the JPA cache, or an empty list if no such entities exist.
	 */
	@Query("select r from CFBamJpaScope r where r.requiredTenantId = :tenantId")
	List<CFBamJpaServerObjFunc> findByTenantIdx(@Param("tenantId") CFLibDbKeyHash256 requiredTenantId);
	 ...
	 */
	default List<CFBamJpaServerObjFunc> findByTenantIdx(ICFBamScopeByTenantIdxKey key) {
	...
	CFBamJpaServerObjFunc findByUNameIdx(@Param("tableId") CFLibDbKeyHash256 requiredTableId,
	 ...
	 *		@return The found entity, typically from the JPA cache, or null if no such entity exists.
	 */
	default CFBamJpaServerObjFunc findByUNameIdx(ICFBamServerMethodByUNameIdxKey key) {
	...
	List<CFBamJpaServerObjFunc> findByMethTableIdx(@Param("tableId") CFLibDbKeyHash256 requiredTableId);
	 ...
	 *		@return The found entity list, which may be empty, typically populated from the JPA cache.
	 */
	default List<CFBamJpaServerObjFunc> findByMethTableIdx(ICFBamServerMethodByMethTableIdxKey key) {
	...
	List<CFBamJpaServerObjFunc> findByDefSchemaIdx(@Param("defSchemaId") CFLibDbKeyHash256 optionalDefSchemaId);
	 ...
	 *		@return The found entity list, which may be empty, typically populated from the JPA cache.
	 */
	default List<CFBamJpaServerObjFunc> findByDefSchemaIdx(ICFBamServerMethodByDefSchemaIdxKey key) {
		return( findByDefSchemaIdx(key.getOptionalDefSchemaId()));
	}

	// CFBamJpaServerObjFunc specified index readers

	/**
	 *	Read zero or more entities into a List using the columns of the CFBamServerObjFuncByRetTblIdxKey as arguments.
	 *
	 *		@param optionalRetTableId
	 *
	 *		@return List&lt;CFBamJpaServerObjFunc&gt; of the found entities, typically from the JPA cache, or an empty list if no such entities exist.
	 */
	@Query("select r from CFBamJpaServerObjFunc r where r.optionalLookupRetTable.requiredId = :retTableId")
	List<CFBamJpaServerObjFunc> findByRetTblIdx(@Param("retTableId") CFLibDbKeyHash256 optionalRetTableId);

	/**
	 *	CFBamServerObjFuncByRetTblIdxKey entity list reader convenience method for object-based access.
	 *
	 *		@param key The CFBamServerObjFuncByRetTblIdxKey instance to use for the query arguments.
	 *
	 *		@return The found entity list, which may be empty, typically populated from the JPA cache.
	 */
	default List<CFBamJpaServerObjFunc> findByRetTblIdx(ICFBamServerObjFuncByRetTblIdxKey key) {
	...
	@Query("select r from CFBamJpaScope r where r.requiredId = :id")
	CFBamJpaServerObjFunc lockByIdIdx(@Param("id") CFLibDbKeyHash256 requiredId);
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFBamJpaScope r where r.requiredTenantId = :tenantId")
	List<CFBamJpaServerObjFunc> lockByTenantIdx(@Param("tenantId") CFLibDbKeyHash256 requiredTenantId);
	 ...
	 *		@return A list of locked entities, refreshed from the data store, or an empty list if no such entities exist.
	 */
	default List<CFBamJpaServerObjFunc> lockByTenantIdx(ICFBamScopeByTenantIdxKey key) {
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFBamJpaServerMethod r where r.requiredContainerForTable.requiredId = :tableId and r.requiredName = :name")
	CFBamJpaServerObjFunc lockByUNameIdx(@Param("tableId") CFLibDbKeyHash256 requiredTableId,
		@Param("name") String requiredName);
	 ...
	 *		@return The locked entity, refreshed from the data store, or null if no such entity exists.
	 */
	default CFBamJpaServerObjFunc lockByUNameIdx(ICFBamServerMethodByUNameIdxKey key) {
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFBamJpaServerMethod r where r.requiredContainerForTable.requiredId = :tableId")
	List<CFBamJpaServerObjFunc> lockByMethTableIdx(@Param("tableId") CFLibDbKeyHash256 requiredTableId);
	 ...
	 *		@return A list of locked entities, refreshed from the data store, or an empty list if no such entities exist.
	 */
	default List<CFBamJpaServerObjFunc> lockByMethTableIdx(ICFBamServerMethodByMethTableIdxKey key) {
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFBamJpaServerMethod r where r.optionalLookupDefSchema.requiredId = :defSchemaId")
	List<CFBamJpaServerObjFunc> lockByDefSchemaIdx(@Param("defSchemaId") CFLibDbKeyHash256 optionalDefSchemaId);
	 ...
	 *		@return A list of locked entities, refreshed from the data store, or an empty list if no such entities exist.
	 */
	default List<CFBamJpaServerObjFunc> lockByDefSchemaIdx(ICFBamServerMethodByDefSchemaIdxKey key) {
		return( lockByDefSchemaIdx(key.getOptionalDefSchemaId()));
	}

	// CFBamJpaServerObjFunc specified delete-by-index methods
	...
	@Lock(LockModeType.WRITE)
	@Query("select r from CFBamJpaServerObjFunc r where r.optionalLookupRetTable.requiredId = :retTableId")
	List<CFBamJpaServerObjFunc> lockByRetTblIdx(@Param("retTableId") CFLibDbKeyHash256 optionalRetTableId);

	/**
	 *	CFBamServerObjFuncByRetTblIdxKey based lock method for object-based access.
	 *
	 *		@param key The key of the entity to be locked.
	 *
	 *		@return A list of locked entities, refreshed from the data store, or an empty list if no such entities exist.
	 */
	default List<CFBamJpaServerObjFunc> lockByRetTblIdx(ICFBamServerObjFuncByRetTblIdxKey key) {
	...
	@Transactional
	@Modifying
	@Query("delete from CFBamJpaServerObjFunc r where r.optionalLookupRetTable.requiredId = :retTableId")
	void deleteByRetTblIdx(@Param("retTableId") CFLibDbKeyHash256 optionalRetTableId);

	/**
	 *	CFBamServerObjFuncByRetTblIdxKey based lock method for object-based access.
	 *
	 *		@param key The CFBamServerObjFuncByRetTblIdxKey of the entity to be locked.
	 */
	default void deleteByRetTblIdx(ICFBamServerObjFuncByRetTblIdxKey key) {

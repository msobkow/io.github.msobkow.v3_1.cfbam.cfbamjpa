// Description: Java 25 JPA implementation of UInt16Col history objects
...
/**
 *  CFBamJpaUInt16ColH provides history objects matching the CFBamUInt16Col change history.
 */
@Entity
@Table(
    name = "uint16col_h", schema = "CFBam31",
    indexes = {
        @Index(name = "UInt16ColIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true),
        @Index(name = "UInt16ColTableIdx_h", columnList = "TableId", unique = false)
    }
)
@Inheritance(strategy = InheritanceType.JOINED)
@DiscriminatorValue("43139")
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFBamPU")
public class CFBamJpaUInt16ColH extends CFBamJpaUInt16DefH
    implements ICFBamUInt16ColH
{
    ...
    public void set( ICFBamValue src ) {
		if( src instanceof ICFBamUInt16Col ) {
			setUInt16Col( (ICFBamUInt16Col)src );
		}
		else {
    ...
    public String toString() {
        String ret = "<CFBamJpaUInt16ColH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}

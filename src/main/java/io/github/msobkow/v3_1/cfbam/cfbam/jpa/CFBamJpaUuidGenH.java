// Description: Java 25 JPA implementation of UuidGen history objects
...
/**
 *  CFBamJpaUuidGenH provides history objects matching the CFBamUuidGen change history.
 */
@Entity
@Table(
    name = "idguuid_h", schema = "CFBam31",
    indexes = {
        @Index(name = "UuidGenIdIdx_h", columnList = "auditClusterId, auditStamp, auditAction, requiredRevision, auditSessionId, Id", unique = true)
    }
)
@Inheritance(strategy = InheritanceType.JOINED)
@DiscriminatorValue("43144")
@Transactional(Transactional.TxType.SUPPORTS)
@PersistenceContext(unitName = "CFBamPU")
public class CFBamJpaUuidGenH extends CFBamJpaUuidTypeH
    implements ICFBamUuidGenH
{
    ...
    public void set( ICFBamValue src ) {
		if( src instanceof ICFBamUuidGen ) {
			setUuidGen( (ICFBamUuidGen)src );
		}
		else {
    ...
    public String toString() {
        String ret = "<CFBamJpaUuidGenH" + getXmlAttrFragment() + "/>";
        return( ret );
    }
}
